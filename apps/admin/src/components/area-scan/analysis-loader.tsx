      ...
      <Card className="w-full max-w-xl mx-4 border  bg-transparent border-none">
        <CardContent className="p-8 md:p-12 flex flex-col items-center text-center space-y-8">
          ...
          <div className="relative">
            <div
              className={`absolute inset-0 rounded-full animate-ping opacity-20 ${currentStepData.bg.replace("/10", "/30")}`}
            ></div>
            <div
              className={`h-24 w-24 rounded-full ${currentStepData.bg} ${currentStepData.border} border-4 flex items-center justify-center transition-all duration-500 transform scale-100`}
            >
              <currentStepData.icon
                className={`h-10 w-10 ${currentStepData.color} transition-all duration-500`}
              />
            </div>
          ...
          <div className="w-full space-y-2">
            ...
            <div className="flex justify-between text-[10px] text-muted-foreground font-mono uppercase tracking-widest">
              ...
              <span>
                Est. Time: {Math.max(0, 10 - progress / 10).toFixed(1)}s
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-4 gap-2 w-full pt-4 opacity-50">
            {steps.map((s, i) => (
              <div
                key={i}
                className={`h-1 rounded-full transition-all duration-300 ${i <= step ? s.color.replace("text-", "bg-") : "bg-muted"}`}
              />
            ))}
          </div>
        </CardContent>
