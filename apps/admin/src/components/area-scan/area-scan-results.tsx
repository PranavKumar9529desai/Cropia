      ...
      <Card className="border-dashed">
        <CardContent className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <p className="text-sm text-muted-foreground max-w-xs mt-2">
            Run an AI analysis to see insights and statistics for this
            jurisdiction.
          </p>
        </CardContent>
      </Card>
  ...
  const getHeadlineStyle = (text: string) => {
    ...
    if (
      ...
      return {
        ...
        containerClass:
          "bg-green-500/5 hover:bg-green-500/10 border-l-2 border-l-green-500",
    ...
    return {
      ...
      containerClass:
        "bg-muted/30 hover:bg-muted/50 border-l-2 border-l-blue-500",
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
          {headlines.map((headline, index) => {
            ...
            return (
              <div
                key={index}
                className={`flex items-start gap-3 p-3 rounded-r-md transition-colors ${style.containerClass}`}
              >
                <div className="mt-0.5 shrink-0">{style.icon}</div>
                <p
                  className={`text-sm font-medium leading-relaxed ${style.textClass}`}
                >
                  {headline}
                </p>
              </div>
        ...
        <div className="md:col-span-12 lg:col-span-8">
          <Card className="h-full">
            <CardHeader>
              ...
              <CardDescription>
                Breakdown of disease detection and healthy crop signs
              </CardDescription>
            </CardHeader>
                    ...
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="text-muted-foreground font-mono">
                        {stats.totalScansAnalyzed > 0
                          ? Math.round(
                              (item.count / stats.totalScansAnalyzed) * 100,
                            )
                          : 0}
                        %
                      </span>
        ...
        <div className="md:col-span-12 lg:col-span-4 space-y-6">
          ...
          <Card className="bg-green-500/[0.03] border-green-500/10">
            <CardHeader className="pb-2">
              ...
              <CardTitle className="text-3xl font-bold flex items-center justify-between">
                {stats.diseaseDistribution.find((d) => d.name === "Healthy")
                  ?.count || 0}
          ...
          <Card className="bg-orange-500/[0.03] border-orange-500/10">
            <CardHeader className="pb-2">
              ...
              <CardTitle className="text-3xl font-bold flex items-center justify-between">
                {
                  stats.diseaseDistribution.filter((d) => d.name !== "Healthy")
                    .length
                }
      ...
      {analysis.createdAt && (
        <div className="flex items-center justify-center gap-2 text-[11px] text-muted-foreground pt-4 opacity-60">
          ...
          Insights engine powered by Gemini 1.5 Flash • Generated{" "}
