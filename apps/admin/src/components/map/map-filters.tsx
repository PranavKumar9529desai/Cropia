...
export default function MapFilters({ data }: MapFiltersProps) {
  ...
  const activeFilterCount = [search.crop, search.disease, search.status].filter(
    Boolean,
  ).length;
    ...
    return (
      <div className="flex items-center justify-between gap-3 px-1">
        <Sheet open={open} onOpenChange={setOpen}>
          <SheetTrigger asChild>
            ...
            >
              <div className="flex items-center gap-2">
                <Filter
                  className={`h-4 w-4 ${hasFilters ? "text-primary" : "text-muted-foreground"}`}
                />
                <span className="font-semibold">Map Filters</span>
                {hasFilters && (
                  <Badge
                    variant="default"
                    className="ml-1 h-5 px-1.5 min-w-[20px] bg-primary text-primary-foreground border-none"
                  >
                    {activeFilterCount}
                  </Badge>
                )}
          ...
          <SheetContent
            side="bottom"
            className="rounded-t-[2rem] p-6 pt-2 pb-10"
          >
