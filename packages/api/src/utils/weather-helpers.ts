 ...
 */
export const calculateWaterBalance = (dailyData: OpenMeteoResponse['daily']) => {
  ...
  let advice = "";
  if (netBalance < -10) advice = "Critical deficit. Irrigation highly recommended.";
  else if (netBalance < 0) advice = "Slight deficit. Monitor crops.";
  else advice = "Soil is moisture positive. No irrigation needed.";
 ...
 */
export const analyzeSprayConditions = (hourly: OpenMeteoResponse['hourly'], currentISOTime: string) => {
  const currentHourIndex = hourly.time.findIndex((t) => t >= currentISOTime);

  if (currentHourIndex === -1) return { canSprayNow: false, nextWindow: "No data" };
  ...
  for (let i = currentHourIndex; i < limit; i++) {
    ...
    next12Hours.push({
      time,
      isSafe,
      reason: isSafe ? "Good conditions" : (wind >= 15 ? "Too windy" : "Rain risk")
    });
  }
 ...
 */
export const analyzeRootHealth = (surfaceMoisture: number, deepMoisture: number) => {
  ...
  if (surfaceMoisture < SURFACE_DRY_THRESHOLD && deepMoisture > DEEP_HEALTHY_THRESHOLD) {
    return {
      status: "SAVER",
      title: "False Dryness",
