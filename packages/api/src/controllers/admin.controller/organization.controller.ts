...
export const OrganizationController = new Hono<{
  ...
  .get("/dashboard", async (c) => {
    ...
    const uniqueVillages = new Set(scans.map((s) => s.village).filter(Boolean))
      .size;

    // 3. Scan Trends
    const lastNDays = Array.from({ length: days })
      .map((_, i) => {
    ...
    const trends = Object.entries(trendsMap).map(([date, scans]) => ({
      date,
      scans,
    }));
    ...
    scans.forEach((scan) => {
      const key =
        ((scan as Record<string, unknown>)[groupLevel] as string) || "Unknown";
    ...
    const health = [
      {
        name: "Healthy",
        value: Math.round((healthMap.Healthy / totalForHealth) * 100),
      },
      {
        name: "Warning",
        value: Math.round((healthMap.Warning / totalForHealth) * 100),
      },
      {
        name: "Critical",
        value: Math.round((healthMap.Critical / totalForHealth) * 100),
      },
    ];
